...
public class RawSocket {
	 ...
	 *                    address. On platforms other than Microsoft Windows, the
	 ...
	 * @param protocol       The protocol within the protocol family.
	 *                       {@link #getProtocolByName} should be used to obtain
	 *                       protocol numbers.
	 * @exception IllegalStateException If the object instance is already open.
	 ...
	 *
	 * @param device The name of the device to bind (e.g., "eth0"). Passing a
	 *               zero-length string will remove the current binding. The
	 *               loopback interface ("lo") and device aliases (e.g., "eth0:1")
	 *               cannot be bound.
	 * @exception IllegalStateException         If the socket has not been opened
	 *                                          first.
	 * @exception UnsupportedOperationException If binding a device name is not
	 *                                          supported on the runtime platform.
	 ...
	 * </p>
	 *
	 * @param useSelect true if {@code select} should be used to implement timeouts,
	 *                  false if not.
	 */
	...
	private native static int __getIfIndex(int socket, byte[] if_name);

	public int getIfIndex(String if_name) throws Exception {
		int result;
		result = __getIfIndex(__socket, if_name.getBytes("UTF8"));
	...
	private native static byte[] __getIfHwAddr(int socket, byte[] if_name);
	
	public byte[] getIfHwAddr(String if_name) throws Exception {
		byte[] result;
		result = __getIfHwAddr(__socket, if_name.getBytes("UTF8"));
	 ...
	 * @param offset  The offset into the buffer where the data should be stored.
	 * @param length  The number of bytes to read.
	 * @param address A byte array in which to store the source address of the
	 *                received packet. It may be null if you don't want to retrieve
	 *                the source address. Otherwise, it must be the right size to
	 *                store the address (e.g., 4 bytes for an IPv4 address).
	 * @exception IllegalArgumentException If the offset or lengths are invalid or
	 *                                     if the address parameter is the wrong
	 *                                     length.
	 * @exception IOException              If an I/O error occurs.
	...
	public int read(byte[] data, int offset, int length, byte[] address)
		...
		if (address != null && ((__family == PF_INET && address.length != 4)
				|| (__family == PF_INET6 && address.length != 16) || (__family == PF_PACKET && address.length != 6)))
